...
public class Model {
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        while (targetY < board.size() && board.tile(x, targetY) == null) {
            targetY++;
        }//结束之后会达到一个非空位置,因为它不满足循环
        if (targetY < board.size() && myValue == board.tile(x, targetY).value()) {
    ...
    public void tilt(Side side) {
        ...
        for (int col = 0; col < board.size(); col++) {
            tiltColumn(col); // 使用 tiltColumn 方法对每列进行操作
        }
